...
def _loan_interest(db: Session, start: date, end: date) -> Decimal:
    ...
    interest = payments_made - (balance_reduction attributable to
    payments, i.e. total reduction minus extra repayments, which are
    100% principal)."""
    ...
    for loan in loans:
        ...
        # _sum_amounts isn't account-scoped, so this is inlined rather
        # than reused — LOAN_PAYMENT needs to stay within this one loan's
        # own account.
        payments_paid = sum(
            (
                t.amount_eur
                for t in db.query(Txn).filter(
                    Txn.account_id == loan.account_id,
                    Txn.type == TransactionType.LOAN_PAYMENT,
                    Txn.voided_at.is_(None),
                    Txn.date > start,
                    Txn.date <= end,
                )
        ...
        total += payments_paid - principal_reduction
    return total
